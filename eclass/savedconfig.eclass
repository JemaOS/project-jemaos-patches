...
save_config() {
	...
	local dest="/usr/local/etc/portage/savedconfig/${CATEGORY}"
	if [[ $# -eq 1 && -f $1 ]] ; then
...
restore_config() {
	...
	local base=${PORTAGE_CONFIGROOT}/usr/local/etc/portage/savedconfig
...
savedconfig_pkg_postinst() {
	...
	if use savedconfig ; then
		...
		find "${EROOT}/usr/local/etc/portage/savedconfig/${CATEGORY}/${PF}" \
