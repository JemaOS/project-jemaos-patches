...
IUSE="
	...
	jemaos_factory_install fixcgroup fixcgroup-memory kvm_host
...
src_prepare() {
  default
  if use jemaos_factory_install; then
    eapply -p2 ${FILESDIR}/insert_factory_install_script.patch
    eapply -p2 ${FILESDIR}/set_default_language_to_zh.patch
    if [ -n "${JEMAOS_FACTORY_INSTALL}" ]; then
      echo $JEMAOS_FACTORY_INSTALL > $JEMAOS_INSTALL_FILE
    fi
  fi
