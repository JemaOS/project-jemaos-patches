...
add_api_keys() {
	...
	local jemaos_api_key=$(awk "/jemaos_api_key/ ${EXTRACT}" "$1")
	local jemaos_client_id=$(awk "/jemaos_default_client_id/ ${EXTRACT}" "$1")
	local jemaos_client_secret=$(awk "/jemaos_default_client_secret/ ${EXTRACT}" "$1")
	...
	BUILD_STRING_ARGS+=(
		...
		"jemaos_default_client_id=${jemaos_client_id}"
		"jemaos_default_client_secret=${jemaos_client_secret}"
	)
}
